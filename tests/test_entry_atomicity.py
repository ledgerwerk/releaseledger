...
def test_entry_apply_preserves_contributors_and_prs(tmp_path: Path) -> None:
    ...
    assert entry.prs == ("github:pr-123",)
    assert entry.contributors == ("@alice",)


def test_entry_apply_rejects_unknown_item_keys(tmp_path: Path) -> None:
    workspace = _init(tmp_path)
    _create_release(workspace)
    with pytest.raises(LaunchError, match=r"items\[0\]\.contributers.*contributors"):
        add_many_release_entries(
            workspace,
            release_version="0.5.0",
            entries=[{
