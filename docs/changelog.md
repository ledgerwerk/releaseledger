...
- Added `repair index` to rebind, rebuild, or quarantine the disposable indexes cache

### Changed

- Changed `status`, `storage where`, and `storage validate` to report storage validity and repair guidance
...
- Added Agent Skill discovery across project, user, and admin locations with metadata validation and conflict diagnostics
...
- Added configurable Git tag ownership with external publication handoff guidance and published-tag validation
...
- Improved release preparation and validation with lifecycle-aware checks, scoped follow-ups, and safe audit handling
...
- Added verified Git ancestry checks for GitHub new-contributor attribution

### Changed

- Changed release validation to detect unacknowledged changelog edits within the reviewed Git range
- Changed release predecessor matching and changelog date checks to use canonical release identities
...
- Added GitHub-aware changelog attribution for contributors, pull requests, and comparison links
- Added safe rebinding for generated index caches before ledger writes

### Changed

- Changed release preparation and changelog builds to support proposed dates and explicit output paths
- Changed release reviews to treat symbolic snapshot drift as advisory while preserving pinned snapshots

### Fixed

- Fixed entry batch validation to reject unknown fields with actionable suggestions
...
- Added release refresh for updating Git snapshots and surfacing pending audit decisions

### Changed

- Changed release checks and tag imports to support target-scoped readiness and explicit history ranges

### Quality

- Improved changelog and entry validation with canonical ordering, separate empty-section controls, and kind-aware linting
...
- Added canonical release workflows with explicit date and Unreleased-content policies
...
- Added safe recovery workflows for canceled releases, version aliases, and atomic bundle and entry moves
...
- Added finalize and published phases for `release check` command
...
- Fixed source ref handling with non-destructive `--add-source-ref` flag
